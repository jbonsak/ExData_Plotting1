...
lines <- 2075259 
size.estimate <- lines / 1000 * top1000.size 
...
par(mfrow=c(2,2))
with(df, {
        plot(df$Global_active_power ~ df$DateTime, ## top left
             type="l",
             xlab="",
             ylab="Global Active Power"
             )  
        
        plot(df$Voltage ~ df$DateTime,            ## top right
             type="l",
             xlab="datetime",
             ylab="Voltage"
             )  
        
        plot(Sub_metering_1 ~ DateTime, type="l", xlab="", ylab="Energy sub metering") ## bottom left
